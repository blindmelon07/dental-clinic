            ...
            <nav role="navigation" aria-label="Main navigation"
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex justify-between items-center h-20">
                        ...
                        {{-- Brand --}}
                        <a href="{{ route('home') }}"
                           class="flex items-center gap-3 flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 rounded-lg"
                           wire:navigate
                           aria-label="{{ $settings->clinic_name }} home">
                            <img src="{{ $settings->logoUrl() }}"
                                 alt="{{ $settings->clinic_name }}"
                                 class="h-11 w-11 object-cover rounded-full ring-2 ring-blue-100">
                        ...
                        <div class="hidden lg:flex items-center gap-1">
                            @foreach ($navLinks as $key => $link)
                                <a href="{{ route($link['route']) }}"
                                   class="px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-150 {{ request()->routeIs($link['route']) ? 'text-blue-800 bg-blue-50' : 'text-slate-600 hover:text-blue-800 hover:bg-blue-50' }}"
                                   wire:navigate
                        ...
                        <div class="flex items-center gap-3">
                            <div class="hidden lg:flex items-center gap-2 pr-3 border-r border-slate-200">
                                <span class="flex items-center justify-center w-9 h-9 rounded-full bg-blue-50 text-blue-700">
                            ...
                            <a href="{{ auth()->check() ? url('/') : route('login') }}"
                               class="hidden lg:inline-flex text-sm font-medium text-slate-600 hover:text-blue-800"
                               wire:navigate>
                                {{ auth()->check() ? 'My Account' : 'Sign In' }}
                            </a>

                            <a href="{{ auth()->check() ? url('/') : route('register') }}"
                               class="hidden sm:inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-blue-700 text-white text-sm font-semibold rounded-xl hover:bg-blue-800 focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 transition-all duration-150 shadow-sm"
                               wire:navigate>
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            ...
                            <button @click="mobileOpen = !mobileOpen"
                                    ...
                                    class="lg:hidden p-2 rounded-xl text-slate-600 hover:bg-slate-100 transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600">
                ...
                <div id="mobile-nav"
                    ...
                    <div class="px-4 py-3 space-y-1">
                        @foreach ($navLinks as $key => $link)
                            <a href="{{ route($link['route']) }}"
                               class="block px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs($link['route']) ? 'text-blue-800 bg-blue-50' : 'text-slate-700 hover:bg-slate-50' }}"
                        ...
                        <div class="pt-3 mt-2 border-t border-slate-100 flex flex-col gap-2">
                            ...
                            <a href="{{ auth()->check() ? url('/') : route('register') }}"
                               class="flex items-center justify-center gap-2 w-full py-2.5 bg-blue-700 text-white text-sm font-semibold rounded-xl hover:bg-blue-800"
                               wire:navigate @click="mobileOpen = false">
                                Book Appointment
                            </a>
                        </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
                    ...
                    <div class="sm:col-span-2 lg:col-span-1">
                        ...
                        @if($settings->facebook_url)
                            <a href="{{ $settings->facebook_url }}" target="_blank" rel="noopener noreferrer"
                               class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-slate-800 text-slate-300 hover:bg-blue-700 hover:text-white transition-colors duration-150 mt-4"
                    ...
                    <div>
                        ...
                        <ul class="space-y-2.5 text-sm">
                            @foreach ($navLinks as $key => $link)
                                <li>
                                    <a href="{{ route($link['route']) }}" class="hover:text-blue-400 transition-colors duration-150" wire:navigate>{{ $link['label'] }}</a>
                                </li>
                            @endforeach
                            <li>
                                <a href="{{ route('login') }}" class="hover:text-blue-400 transition-colors duration-150" wire:navigate>Patient Portal</a>
                            </li>
                        </ul>
                    </div>
                        ...
                        <ul class="space-y-3 text-sm">
                            @if($settings->address || $settings->city)
                                <li class="flex items-start gap-2.5">
                                    <svg class="w-4 h-4 mt-0.5 text-blue-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        function initScrollReveal() {
            ...
            targets.forEach(function (el, i) {
                if (!el.hasAttribute('data-sr')) {
                    ...
                    el.style.transitionDelay = Math.min(i * 0.15, 0.45) + 's';
                }
            });
