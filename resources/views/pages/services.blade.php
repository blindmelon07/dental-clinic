<x-layouts.guest title="Services">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @forelse ($categories as $category)
                <div class="mb-14">
                    ...
                    <div class="flex items-center gap-3 mb-6">
                        <div class="flex items-center justify-center w-10 h-10 rounded-xl bg-indigo-100 text-indigo-600 flex-shrink-0">
                    ...
                    @if($category->services->isNotEmpty())
                        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach ($category->services as $service)
                                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                                    {{-- Icon --}}
                                    <div class="flex items-center justify-center w-14 h-14 rounded-2xl bg-indigo-50 text-indigo-500 mb-5 text-2xl">
                                    ...
                                    {{-- Name --}}
                                    <h3 class="font-heading text-xl font-bold text-indigo-700">{{ $service->name }}</h3>
                                    ...
                                    <a href="{{ auth()->check() ? url('/') : route('register') }}"
                                       class="mt-4 inline-flex items-center justify-center w-full px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white text-sm font-semibold rounded-xl hover:from-indigo-600 hover:to-indigo-700 shadow-sm shadow-indigo-200 transition-all duration-150"
                                       wire:navigate>
                                        Book Appointment
                                    </a>
                                </div>
            ...
            @empty
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm text-center py-16">
                    ...
                    <a href="{{ route('contact') }}" class="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-indigo-600 hover:text-indigo-700" wire:navigate>
                        Contact Us
    ...
    <section class="py-12 bg-white border-t border-slate-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <div class="bg-indigo-50 rounded-2xl p-6">
                    ...
                    <ul class="mt-4 space-y-2.5">
                        <li class="flex items-start gap-2.5 text-sm text-slate-700">
                            <svg class="w-4 h-4 text-indigo-500 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                ...
                <div class="bg-slate-50 rounded-2xl p-6 flex flex-col justify-between">
                    <div>
                ...
                <div class="bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-2xl p-6 flex flex-col justify-between">
                    <div>
                        <h3 class="font-heading text-lg font-bold text-white">Ready to Book?</h3>
                        <p class="mt-2 text-sm text-indigo-100">Schedule your appointment online in just a few minutes.</p>
                    </div>
                    <a href="{{ auth()->check() ? url('/') : route('register') }}"
                       class="mt-5 inline-flex items-center justify-center gap-2 w-full px-5 py-3 bg-white text-indigo-700 text-sm font-semibold rounded-xl hover:bg-indigo-50 transition-all duration-150"
                       wire:navigate>
                        Book Appointment
                    </a>
                </div>
    ...
    {{-- Trust badges strip --}}
    <section class="py-12 bg-teal-50/60 border-t border-teal-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-8 text-center">
                <div>
                    <p class="font-heading text-3xl font-extrabold text-teal-600">{{ $settings->stat_years ?: '15+' }}</p>
                    <p class="mt-1 text-sm text-slate-500">Years Experience</p>
                </div>
                <div>
                    <p class="font-heading text-3xl font-extrabold text-teal-600">{{ $settings->stat_patients ?: '10k+' }}</p>
                    <p class="mt-1 text-sm text-slate-500">Happy Patients</p>
                </div>
                <div>
                    <p class="font-heading text-3xl font-extrabold text-teal-600">{{ $settings->stat_satisfaction ?: '98%' }}</p>
                    <p class="mt-1 text-sm text-slate-500">Satisfaction Rate</p>
                </div>
                <div>
                    <p class="font-heading text-3xl font-extrabold text-teal-600">{{ $settings->stat_emergency ?: '24/7' }}</p>
